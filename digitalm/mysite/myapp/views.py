...
@csrf_exempt
def create_checkoout_session(request,id):
...
def payment_success_view(request):
    ...
    order=get_object_or_404(OrderDetail,stripea_payment_intent=session.payment_intent)
    order.has_paid=True
...
def dashbord(request):
    ...
    return render(request,'myapp/dashbord.html',{'products':products})
...
def sales(request):
    ...
    data=OrderDetail.objects.filter(product__seller=request.user,created_on=last_year)
    yearly_sales=data.aggregate(Sum('amount'))

    #30 days
    last_month=datetime.date.today() - datetime.timedelta(days=30)
    data=OrderDetail.objects.filter(product__seller=request.user,created_on=last_month)
    monthly_sales=data.aggregate(Sum('amount'))

    #7 days
    last_week=datetime.date.today() - datetime.timedelta(days=7)
    data=OrderDetail.objects.filter(product__seller=request.user,created_on=last_week)
